<template>
  <div class="relative py-0.5 px-3">
    ...
    >
      <div class="mb-1 image bg-contain min-w-12">
        <img class="mb-1 h-full w-12" :src="film.posterUrlPreview ? film.posterUrlPreview : film.posterUrl"
             :alt="film.nameRu ? film.nameRu : film.nameEn">
      </div>
      <div class="pr-8">
        <h3 class="text-xs mb-1">{{ film.nameRu ? film.nameRu : film.nameEn }}</h3>
        <h4 v-show="film.year && film.year !== 'null'" class="text-xs text-slate-400 mb-1">{{ film.year }}</h4>
      </div>
    </RouterLink>
    <div
    ...
    <XMarkIcon class="absolute w-7 p-1 text-gray-400 top-1/2 -translate-y-1/2 right-2" alt="close" v-show="remove"
